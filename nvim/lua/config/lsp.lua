-- Lsp
-- Enable & configure language server protocols
...
local function on_attach(client, bufnr)
    ...
    if client:supports_method 'textDocument/codeAction' then
        require('utils.lightbulb').attach_lightbulb(bufnr, client)
    end

    -- Don't check for the capability here to allow dynamic registration of the request.

    if client:supports_method 'textDocument/references' then
        keymap('grr', '<Cmd>FzfLua lsp_references jump1=true ignore_current_line=true<CR>', 'vim.lsp.buf.references()')
    end

    if client:supports_method 'textDocument/rename' then
        keymap('grn', require 'utils.lsp_renamer', 'vim.lsp.buf.rename()')
    end
    ...
    if client:supports_method 'textDocument/definition' then
        keymap('gd', '<Cmd>FzfLua lsp_definitions jump1=true ignore_current_line=true<CR>', 'Go to definition')
        keymap('gD', '<Cmd>FzfLua lsp_definitions jump1=false ignore_current_line=true<CR>', 'Peek definition')
    end
    ...
    if client:supports_method 'textDocument/documentHighlight' then
        local under_cursor_highlights_group = vim.api.nvim_create_augroup('bugs/cursor_highlights', { clear = false })
    ...
    if client:supports_method 'textDocument/inlayHint' then
        local inlay_hints_group = vim.api.nvim_create_augroup('bugs/toggle_inlay_hints', { clear = false })
