-- Renamer
-- Beautiful renamer
-- Creadit: https://github.com/NvChad/ui/blob/v3.0/lua/nvchad/lsp/renamer.lua
local lsp = vim.lsp
local api = vim.api
...
return function()
    get_symbol_to_rename(function(to_rename)
        ...
        local winopts = {
            ...
            relative = 'cursor',
            width = #to_rename + 15,
            title = { { ' ' .. require('utils.icons').misc.renamer .. ' Renamer ', 'LspRenamerTitle' } },
            title_pos = 'center',
        }

        local win = api.nvim_open_win(buf, true, winopts)
        vim.wo[win].winhl = 'Normal:LspRenamerNormal,FloatBorder:LspRenamerBorder'
        api.nvim_set_current_win(win)
        api.nvim_buf_set_lines(buf, 0, -1, true, { ' ' .. to_rename })

        vim.bo[buf].buftype = 'prompt'
        vim.bo[buf].filetype = 'lsp_renamer'
